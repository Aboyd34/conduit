  ...
  return (
    <div className="you-view">
      <div className="you-header">
        ...
        <div>
          <h2 className="view-title you-handle">{myPubkey ? myPubkey.slice(0,12)+'…' : 'Anonymous'}</h2>
      ...
      <div className="you-actions">
        <button className={`you-toggle-btn ${showKeys ? 'you-toggle-btn--active' : ''}`} onClick={() => setShowKeys(v=>!v)}>
      ...
      <div className="you-posts-section">
        ...
        {myPosts.length === 0 ? (
          ...
          <div className="feed">{myPosts.map(p => <PostCard key={p.id} post={p} onViewProfile={onViewProfile} />)}</div>
        )}
      </div>
    </div>
  );
