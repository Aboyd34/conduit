...
// ── DB: use file in production, memory in dev ─────────────────────────────────
...
function verifyAgeToken(raw) {
  ...
  try {
    ...
    const sha256sig = crypto.createHash('sha256')
      ...
      .digest('hex');
    const btoaSig = Buffer.from(`conduit:${token.timestamp}:${token.salt}`).toString('base64');
    if (token.sig !== sha256sig && token.sig !== btoaSig)
...
async function startServer() {
  ...
  app.get('/api/dm/:pubkey', requireAgeVerified, async (req, res) => {
    ...
    const msgs = await dbAll(`
      ...
      ORDER BY timestamp ASC LIMIT 200`, [pubkey, withPubkey, withPubkey, pubkey]);
    res.json(msgs);
  });
  app.post('/api/dm', requireAgeVerified, async (req, res) => {
    const { sender_pubkey, recipient_pubkey, content } = req.body;
    if (!sender_pubkey || !recipient_pubkey || !content) return res.status(400).json({ error: 'Missing fields' });
    const id = crypto.randomUUID();
    const ts = Date.now();
  ...
  app.get('/api/peers', requireAgeVerified, async (req, res) => {
    res.json((await dbAll('SELECT * FROM peers ORDER BY last_seen DESC'))
