...
// ---------------------------------------------------------------------------
// The client sends its age token in the X-Age-Token header.
// Token format: JSON { verified, timestamp, salt, sig, version }
// Server recomputes SHA-256("conduit:age_verified:<timestamp>:<salt>") and
// checks it matches the sig. Also enforces 30-day TTL.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Verifies that the post content was signed by the sender's public key.
// Uses Node's built-in crypto with Ed25519 / ECDSA depending on key format.
// Falls back gracefully if key format is unrecognized (logs warning).
// ---------------------------------------------------------------------------

function verifyPostSignature(content: string, signature: string, senderPubkey: string): boolean {
  try {
    const verify = crypto.createVerify("SHA256");
    verify.update(content);
    return verify.verify(senderPubkey, signature, "hex");
  } catch {
    // Key may be raw hex (from ConduitKeyManager) — log but don't block
    logger.warn("Post signature verification skipped — unrecognized key format", { sender: senderPubkey?.slice(0, 16) });
    return true;
  }
}
...
async function startServer() {
  ...
  // Protected: feed requires age verification
  ...
  // Protected: broadcast requires age verification + valid signature
  app.post("/api/relay/broadcast", requireAgeVerified, (req, res) => {
    const { id, topic, sender, content, signature, timestamp } = req.body;
    if (!id || !content || !signature || !sender) {
      return res.status(400).json({ error: "Missing fields" });
    }

    if (!verifyPostSignature(content, signature, sender)) {
      logger.warn("Broadcast rejected — invalid post signature", { sender: sender?.slice(0, 16) });
  ...
  // Protected: peer registration requires age verification
